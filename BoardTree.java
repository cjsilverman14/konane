...
public class BoardTree {//The BoardTree class for storing successors
    ...
    public void fillNodes(BoardNode h, int depth, boolean level) {//Populate the tree with nodes
        ...
        h.fillSuccessors(alg.findSuccessors(h.b,level), depth);//Set the successors of a node
        level = !level;//Change who it is finding successive moves for
        Controller.branches++;
        for(int i = 0; i < h.successors.size(); i++) {
            Controller.branchCount++;
            fillNodes(h.successors.get(i),depth-1, level);//Recursively run this function on successive nodes
        }
